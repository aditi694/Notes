...
//        System.out.println(list2.stream().sorted((a,b)->(b-a)).toList());
        ...
        Map<Integer, List<String>> groupBy =
                word1.stream()
...
//        List<Integer> duplicateFind = list1.stream().filter(n -> !seen.add(n)).distinct().toList();
        List<Integer> duplicates =
                list1.stream()
                        .filter(n -> Collections.frequency(list1, n) > 1)
                        .distinct()
                        .toList();
        ...
        List<List<Integer>> listof =
                Arrays.asList(Arrays.asList(1, 2, 3), listB, listC);
        ...
        Map<Boolean, List<Integer>> partionedBy =
                method.stream()
        ...
        List<String> duplicateWords =
                Arrays.asList("java", "boot", "grapes", "spring", "boot", "java", "grapes");
...
//        Map<String, Long> freqMap = duplicateWords.stream().collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
        ...
        List<Integer> mergedList =
                Stream.concat(lista.stream(), listb.stream()).toList();
        System.out.println(mergedList);

        //        to find first element
        List<String> first =
                Arrays.asList("java", "spring", "Aboot", "grapes");
        first.stream()
                .findFirst()
...
//        System.out.println(first.get(0));
        ...
        List<String> filterWord =
                Arrays.asList("java", "spring", "Aboot", "grapes");
        List<String> restWord =
                filterWord.stream()
                        .filter(n -> n.toLowerCase().contains("p"))
                        .toList();
        ...
        //    length wise strings
        List<String> lenWise =
                Arrays.asList("apple", "banana", "kiwi", "oranges", "grapesss");
...
//        List<String> sortedLen = lenWise.stream().sorted(Comparator.comparing(String::length)).toList();
        ...
        //        palindrome strings
        List<String> palindromeStrings =
                Arrays.asList("madam", "apple", "racecar", "banana", "level");
        List<String> checkString =
                palindromeStrings.stream()
                        .filter(n ->
                                new StringBuilder(n)
                                        .reverse()
                                        .toString()
